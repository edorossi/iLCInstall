...
class BaseILC:
    ...
    def writeLocalEnv(self):
        ...
        #protect against double-sourcing (starting part)
        if string.upper(self.name) in self.env.keys():
            f.write( 'if [ -n "$'+string.upper(self.name)+'" ]; then\n')
            f.write( "\techo '!!! "+self.name+" already sourced to '$"+string.upper(self.name)+"' !!!'\n")
            f.write( "else\n")
        ...
        #protect against double-sourcing (closing part)
        if string.upper(self.name) in self.env.keys():
            f.write( "\nfi")
